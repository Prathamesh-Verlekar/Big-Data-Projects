...
def upload_files():
    ...
    bucket_name = 'netflix-dataset'

    model_files = [os.path.abspath(os.path.join(os.path.dirname(__file__), '.','dataset', 'netflix_titles.csv'))]

    for file in model_files:
        # -----------------------------------------------------------------------------------------------
        # Example: upload_file(Filename='/tmp/hello.txt', Key='hello.txt')
        # Filename = C:\Users\Prathamesh Verlekar\PycharmProjects\airlfow-basic\dataset\netflix_titles.csv
...
def download_data():
    ...
    bucket_name = s3.Bucket('netflix-dataset')
